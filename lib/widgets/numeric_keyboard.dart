...
class NumericKeypad extends StatefulWidget {
  const NumericKeypad(
      {super.key, required this.controller, required this.onNumberPressed, required this.onAcceptPressed});

  final TextEditingController controller;
  final Function(String newInput) onNumberPressed;
...
class _NumericKeypadState extends State<NumericKeypad> {
  ...
  void _onSelectionChanged() {
    log(_controller.text, name: '_onSelectionChanged');
    if (_controller.text.length < 5) {
      setState(() {
        // update selection on change (updating position too)

        _selection = _controller.selection;
        widget.onNumberPressed(_controller.text);
      });
    }
    ...
    return Column(
      children: [
        ...
        Expanded(
          child: Row(
            children: [
              _buildButton(AppLocalizations.of(context)!.done, onPressed: widget.onAcceptPressed),
              _buildButton('0'),
  ...
  void _backspace() {
    ...
    if (value.isNotEmpty && position != 0) {
      ...
      _controller.selection = TextSelection.fromPosition(TextPosition(offset: position - 1)); // 4) update the cursor
    }
  }
}
